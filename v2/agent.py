...
from .types import TaskShape, load_state, save_state, DEBUG
...
@graph_fn(
    ...
    as_agent={
        ...
        "short_description": "Task-shape routed DeepLens agent with bounded tool use and background run support.",
        "description": "A simplified v2 DeepLens agent with a minimal router, stronger memory/context assembly, and approval-aware tool dispatch.",
        "icon_key": "microscope",
        "color": "teal",
        "mode": "chat_v1",
...
async def deeplens_agent(
    ...
    *,
    ...
    if not raw_message and not attachments:
        return {
            "reply": (
                ...
                "Try `/simulate`, `/optimize`, `/mode full`, or `/mode lite`."
            )
        }
    ...
    if route_result["immediate_reply"]:
        ...
        reply = (
            "This DeepLens agent currently supports standard simulation, optimization submission, "
            "run status or cancellation, and bounded conceptual help tied to those flows."
        )
    else:
    ...
    await mem.record_chat_assistant(
        text=reply,
        tags=["ag.deeplens.v2.reply"],
        data={"task_shape": decision["task_shape"].value},
    )
    return {"reply": reply}
